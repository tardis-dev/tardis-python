...
async def fetch_data_to_replay(exchange, from_date, to_date, filters, endpoint, cache_dir, api_key):
    ...
    while offset < minutes_diff:
        ...
        fetch_data_tasks.add(
            asyncio.create_task(
                _fetch_data_if_not_cached(endpoint, headers, cache_dir, exchange, from_date, offset, filters)
            )
        )
        offset += 1
...
async def _fetch_data_if_not_cached(endpoint, headers, cache_dir, exchange, from_date, offset, filters):
    ...
    if os.path.isfile(cache_path) == False:
        await _reliably_fetch_and_cache_slice(endpoint, headers, exchange, from_date, offset, filters, cache_path)
...
async def _reliably_fetch_and_cache_slice(endpoint, headers, exchange, from_date, offset, filters, cache_path):
    ...
    while True:
        ...
        try:
            await _fetch_and_cache_slice(fetch_url, headers, cache_path)
            break
...
async def _fetch_and_cache_slice(url, headers, cache_path):
